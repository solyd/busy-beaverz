...
void init() {
  ...
  for (int m = 0; m < min(mm.num_symbols+1, M); m++) {
    mm.add_tf(S('b'), 0, m, D('l'), S('a')); add_queue; ms.push(mm);
    mm.add_tf(S('b'), 0, m, D('l'), S('b')); add_queue; ms.push(mm);
  }

  if (N >= 3) {
    for (int m = 0; m < min(mm.num_symbols+1, M); m++) {
      mm.add_tf(S('b'), 0, m, D('l'), S('c')); add_queue; ms.push(mm);
      mm.add_tf(S('b'), 0, m, D('r'), S('c')); add_queue; ms.push(mm);
    }
  }
}
...
// doesn't work, just for myself to make code clearer
optional<machine> pop_queue() {
  mut.lock();
  if (ms.size() == 0) {
    mut.unlock();
    return none;
